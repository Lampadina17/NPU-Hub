...
(function (g, f) {
    ...
    var Le = ((l = "") => {
        ...
        De = ue(/ {0,3}(?:[*+-]|1[.)])[ \t]+[^ \t\n]/), ve = ue(/ {0,3}(?:[*+-]|\d{1,9}[.)])(?:[ \t]|\n|$)/),
        He = d(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph", ve).getRegex(), X = {
            blockquote: He,
            code: Me,
            def: Ie,
            fences: ze,
        ...
        pe = /^( {2,}|\\)\n(?!\s*$)/,
        je = /^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/, C = /[\p{P}\p{S}]/u,
        Z = /[\s\p{P}\p{S}]/u, J = /[^\s\p{P}\p{S}]/u,
        Fe = d(/^((?![*_])punctSpace)/, "u").replace(/punctSpace/g, Z).getRegex(), ce = /(?!~)[\p{P}\p{S}]/u,
        Ue = /(?!~)[\s\p{P}\p{S}]/u, Ke = /(?:[^\s\p{P}\p{S}]|~)/u,
        We = d(/link|precode-code|html/, "g").replace("link", /\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-", Le ? "(?<!`)()" : "(^^|[^`])").replace("code", /(?<b>`+)[^`]+\k<b>(?!`)/).replace("html", /<(?! )[^<>]*?>/).getRegex(),
        he = /^(?:\*+(?:((?!\*)punct)|([^\s*]))?)|^_+(?:((?!_)punct)|([^\s_]))?/,
        Xe = d(he, "u").replace(/punct/g, C).getRegex(), Je = d(he, "u").replace(/punct/g, ce).getRegex(),
        ...
        }, D = {normal: X, gfm: Ze, pedantic: Ge}, A = {normal: V, gfm: j, breaks: ct, pedantic: pt};
    var ht = {"&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;"}, fe = l => ht[l];
...
`, e = e.substring(k.length + 1), a = !0), !a) {
                        let $ = this.rules.other.nextBulletRegex(f), ne = this.rules.other.hrRegex(f),
                            re = this.rules.other.fencesBeginRegex(f), se = this.rules.other.headingBeginRegex(f),
                            Re = this.rules.other.htmlBeginRegex(f), Te = this.rules.other.blockquoteBeginRegex(f);
                        for (; e;) {
                            let G = e.split(`
`, 1)[0], B;
...
`), href: s, title: r
        ...
        table(e) {
            ...
            let n = ee(t[1]), s = t[2].replace(this.rules.other.tableAlignChars, "").split("|"),
...
` ? t[1].slice(0, -1) : t[1];
        ...
        emStrong(e, t, n = "") {
            ...
            if (!(s[1] || s[3] || "") || !n || this.rules.inline.punctuation.exec(n)) {
                let i = [...s[0]].length - 1, o, u, a = i, p = 0,
...
Please report this to https://github.com/markedjs/marked.`, e) {
    ...
    g.defaults = R;
    g.use = function (...l) {
        return z.use(...l), g.defaults = z.defaults, Q(g.defaults), g
    };
    ...
    g.Hooks = P;
    g.parse = g;
    var dt = g.options, gt = g.setOptions, ft = g.use, mt = g.walkTokens, xt = g.parseInline, bt = g, Rt = b.parse,
        Tt = x.lex;
